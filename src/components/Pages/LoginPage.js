...
const LoginPage= ({login}) => {
    ...
    sendQuery(verificationQuery({status : data.get('status'), email : data.get('email')}),true)
            .then(function(res){
                if(res == "New") {message.success("Account not found")}
                  ...
                  sendQuery(verificationQuery({status : data.get('status'), email : data.get('email'), password :data.get('password')},true),true)
                    .then(function(res){
                        if(res == "New") {message.warning("Incorrect Password")}
                        else if(res == "Found") {
  ...
  return (
    <>
    {!register ? <ThemeProvider theme={theme}>
